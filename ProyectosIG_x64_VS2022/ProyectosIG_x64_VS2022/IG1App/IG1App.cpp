	...
	switch (key) {
		case GLUT_KEY_RIGHT:
			if (mdf == GLUT_ACTIVE_CTRL)
				mCamera->truePitch(-2); // rotates -1 on the X axis
			else
				mCamera->truePitch(2); // rotates 1 on the X axis
			break;
		case GLUT_KEY_LEFT:
			if (mdf == GLUT_ACTIVE_CTRL)
				mCamera->trueYaw(2); // rotates 1 on the Y axis
			else
				mCamera->trueYaw(-2); // rotate -1 on the Y axis
			break;
		case GLUT_KEY_UP:
			mCamera->trueRoll(2); // rotates 1 on the Z axis
			break;
		case GLUT_KEY_DOWN:
			mCamera->trueRoll(-2); // rotates -1 on the Z axis
			break;
